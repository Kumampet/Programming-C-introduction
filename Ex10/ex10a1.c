/*
以下はキーボードから入力する2つの2 x 3 行列の和を計算して表示するプログラムである。 下線部および指示された箇所を補い、プログラムを完成させなさい。
なお、行列の要素を格納する配列の行数と列数はそれぞれROW、COLUMNというマクロで定義するものとし、 配列の宣言やfor文の条件式に2や3などの定数値は書かないこととする。
