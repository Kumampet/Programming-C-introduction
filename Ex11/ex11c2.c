...
int main(){
  int dat[MSIZE][MSIZE];
  int i, j, width, height, maxval,shikii;
  /* 入力 */
  /* ファイル形式のチェック */
  ...
  for (i = 0; i < height; i++){
    for (j = 0; j < width; j++){
      if(dat[i][j]<= shikii){ //dat[i][j]がしきい値以下。。。
	dat[i][j] = dat[i][j]/COEF; //暗くする
      }else {
	dat[i][j] = dat[i][j]*COEF; //そうでなければCOEF倍する
	if(dat[i][j] >maxval){ //もしmaxvalを超たら
	  dat[i][j] = maxval;//maxvalに修正
	}
      }
    }
  }
