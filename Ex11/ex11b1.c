/*
以下に、プログラムで画像を上下反転させるための手順とそのプログラムのソースがあります。 プログラム中の下線部を埋めて、完成させなさい。 なお、実行ファイル名は gflpud とすること（以下の実行例とハンドアウトを参照）。

「手順」
画像情報を保持するための２次元配列とその他の変数の宣言
カウンタ変数の宣言
画像サイズ、濃淡情報の読み込み及びエラーチェック
画像データの読み込み及びエラーチェック
データ形式の出力
画像サイズ、濃淡情報の出力
画像データの出力（２重ループを使い、上下を逆にする）
プログラム
